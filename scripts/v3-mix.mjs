...
const fade=ledger.sfxFade??{at:130,duration:2};
...
for(const id of [label]){
  ...
  const filter=`[0:a]apad,atrim=duration=${ledger.duration}[vo];[1:a]apad,atrim=duration=${ledger.duration},volume='1.5*(1-0.72*(${duck}))':eval=frame[m];[2:a]volume='0.60*(1-0.75*(${duck}))':eval=frame,afade=t=out:st=${fade.at}:d=${fade.duration}[s];[vo][m][s]amix=inputs=3:normalize=0:duration=longest,atrim=duration=${ledger.duration}[mix]`;
  run(['-i',voice,'-i',opt('music')??join(root,'music-provisional-150s/v3-symphony-of-gates-provisional-150s.wav'),'-i',opt('sfx')??join(root,'stems/v3-sfx.wav'),'-filter_complex',filter,'-map','[mix]','-ar','48000','-ac','2','-c:a','pcm_f32le',premix]);
