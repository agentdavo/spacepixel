...
/** The NPC clock: the world clock, or the play clock if that's further on (older saves). */
export function npcNow(ledgerClock: number): number {
  return Math.max(world().state.clock, ledgerClock);
}
