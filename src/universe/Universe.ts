...
/** Ground terrain under a surface port (from the planet's kind; gas giants get a cloud sea). */
export type SurfaceTerrain = 'ocean' | 'desert' | 'rocky' | 'ice' | 'volcanic' | 'cloud' | 'lantern';

/**
 * A surface port (planetary ports): the city at the foot of an orbital port's
 * landing tether, reached by the descent corridor. Generated by
 * `universe/surfacePorts.ts` after the survey pass; a market like any station
 * (kind 'surface'), but no StationView — the descent swaps in a local surface
 * scene (`world/surface`).
 */
export interface SurfacePortSite extends StationSite {
  kind: 'surface';
  /** Id of the orbital port whose tether comes down here. */
  orbital: string;
  planet: number;
  terrain: SurfaceTerrain;
  /** One line for the dock screen / star map. */
  description: string;
  /** Radians: heading of the approach below the cloud deck (surface frame). */
  heading: number;
}
...
export interface StarSystem {
  ...
  /** Surface ports under the orbital ports' tethers (inhabited worlds only). */
  surfacePorts?: SurfacePortSite[];
