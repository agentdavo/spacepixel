...
export function outfitScenario(): ScenarioResult {
  ...
  return {
    ...
    checks: [
      ...
      check('… with some margin (hull left %)', valStock.hullLeft * 100, '15..40', valStock.hullLeft >= 0.15 && valStock.hullLeft <= 0.4),
      check('Valiant Mk III fit beats stock (hull left, points)', (valGood.hullLeft - valStock.hullLeft) * 100, '>= 10', valGood.hullLeft - valStock.hullLeft >= 0.1 && valStock.t >= valGood.t),
      check('stock fits are legal on the power budget', Math.max(resStock.power.draw - resStock.power.output, valStock.power.draw - valStock.power.output), '<= 0', resStock.power.draw <= resStock.power.output && valStock.power.draw <= valStock.power.output),
