...
export class FlightScene implements GameScene, FlightHostScene {
  ...
  update({ dt: realDt, time }: FrameContext): void {
    ...
    if (!this.campaign) for (const e of this.weapons.events) if (e.kind === 'kill' && e.ship && e.shooter === this.player && e.ship.team !== 'renegade') this.ledger = reputationForKill(this.ledger, e.ship.faction as EconFaction);
    if (this.mission) {
      this.missionTime += dt;
