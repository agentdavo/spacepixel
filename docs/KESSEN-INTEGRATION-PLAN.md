...
**Current bounded deliverable: accepted environmental cameos in Episodes 10/19, integrated locally and enabled by default.** The chief accepted the revised service modules, local contrast and both final motion samples, then authorised activation after combined navigation checks. All 308 tests/build and native default-on/explicit-off mission/navigation comparisons pass. `kessenCameos=0` remains a debug opt-out. The source branch name `codex/kessen-faction-foundation` records the superseded scope; the only shared type extension is a set-piece kind. General faction/economy/boarding/Train mechanics remain deferred. The proposals below preserve design history; their earlier preview gate is superseded by this acceptance. See [evidence and reproduction notes](KESSEN-CAMEO-PREVIEW.md).
